...
void loop() {
  ...
  esp8266Module.end();
  Serial.println("response: " + espResponse);
  if(espResponse == NULL) {
    espDataIsAvailable = false;
  }else if (espResponse.substring(0,6) == "Unable") {
    espDataIsAvailable = false;
  }else if (espResponse.substring(0,5) == "{\"lt\"") {
    espDataIsAvailable = true;
  }else {
    espDataIsAvailable = false;
  }
  DeserializationError error = deserializeJson(doc, espResponse);
  if (error) {
    Serial.println("there was error");
  }
  JsonObject espResponseData = doc.as<JsonObject>();
  String ed = espResponseData["edu"];
  Serial.println(ed);
  /**
